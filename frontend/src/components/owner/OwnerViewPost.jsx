...
import JobViewTable from '../staff/JobViewTable'
...
import ModalSample from '../staff/utils/ModalSample';
import StaffViewTable from './StaffViewTable';
...
const StyledTableCell = styled(TableCell)(({ theme }) => ({
 ...
 function OwnerViewPost() {
    ...
    const getPostedStaff = async (item) =>{
        await axios.get(`${BaseUrl}/getpostedstaff/${item}`, {
            headers:{
    ...
    const handlePopupOpen =(item, status)=>{
        setJobStatus(status)
        if(status === 'staff')
        getPostedStaff(item)
        else
    ...
    const fetchJobList = async(ownerToken) =>{
      setLoading(true)
      await axios.get(BaseUrl+'/ownerjobposts', {
          headers:{
            'Content-Type':'application/json',
            Authorization:`Bearer  ${ownerToken}`
          }
     ...
     <div>
        ...
        >
                    ...
                    {rows.map((row, index) => {
                      return(
                        ...
                        <StyledTableCell align="right"> 
                          <Button variant="outlined" color="info" onClick={()=>handlePopupOpen(row, 'detail')} sx={{marginRight:'1rem'}}>
                            <OpenInBrowserIcon /> More
                          </Button>
                          <Button variant="outlined" color="info" onClick={()=>handlePopupOpen(row.job_staff, 'staff')} sx={{marginRight:'1rem'}}>
                            <OpenInBrowserIcon /> Staff
                          </Button> 
                            {
                               row.is_active === false?
          ...
          <Popup title={viewJob.username? viewJob.job_title: 'JOB'} openPopup={openPopup} setOpenPopup={setOpenPopup}>
            <StaffViewTable viewJob={viewJob} />
            </Popup>
            :
